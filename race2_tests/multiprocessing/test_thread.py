...
class TestCASSpinlock(_TestCASSpinlock):
    def setUp(self):
        self.thread_generator__dict = {
            0: ThreadGenerator(self.thread_sub_fun),
            1: ThreadGenerator(self.thread_sub_fun),
            2: ThreadGenerator(self.thread_sub_fun),
        }

        for x in self.thread_generator__dict.values():
            x.open()

    def thread_sub_fun(self, locks: CAS, lock_id: int) -> int:
        yield_fun_yield(1)
